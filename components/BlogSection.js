...
export default function BlogSection({ blogs, showAll = false }) {
  // For the homepage (not showAll), get featured blogs
  const featuredBlogs = blogs.filter(blog => blog.featured);
  
  // Select blogs to display
  // On homepage: show 3 blogs on desktop, but only first 2 on mobile
  // On blog page: show all blogs
  const homepageBlogs = featuredBlogs.slice(0, 3);
  const allBlogs = blogs;
  
  const displayBlogs = showAll ? allBlogs : homepageBlogs;
  ...
  return (
    <div className="bg-slate-950 text-white py-24">
      <div className="max-w-7xl mx-auto px-6">
        ...
        {/* Blog Grid - Show 2 on mobile, 3 on desktop when not showAll */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-12">
          {displayBlogs.map((blog, index) => (
            // Hide the third blog card on mobile devices only
            <div 
              key={blog.id} 
              className={(!showAll && index === 2) ? "hidden md:block" : ""}
            >
              <Link 
                href={`/blog/${blog.slug}`}
                className="group bg-slate-900 border-2 border-slate-800 rounded-2xl overflow-hidden hover:border-emerald-500/50 transition-all duration-300 hover:scale-105 hover:shadow-2xl hover:shadow-emerald-500/20 h-full flex flex-col"
              >
                {/* Blog Image */}
